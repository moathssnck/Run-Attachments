...
export default function SystemContentPage() {
  ...
  const { data: sysContentList = [] } = useQuery<
    { systemContentCategoryId: number; lookupId: number }[]
  >({
    ...
    queryFn: async () => {
      ...
      return rows.map((r: Raw) => ({
        systemContentCategoryId: asNum(
          r.systemContentCategoryId ?? r.categoryId,
        ),
  ...
  const sysContentCategoryIds = useMemo(
    () =>
      new Set(
        sysContentList
          .map((c) => c.systemContentCategoryId)
          .filter((id) => id > 0),
      ),
    [sysContentList],
  );
    ...
    () =>
      new Set(
        sysContentList
          .filter(
            (c) =>
              c.systemContentCategoryId === selectedCategoryId &&
              c.lookupId > 0,
          )
          .map((c) => c.lookupId),
      ),
  ...
  const {
    ...
    queryFn: async () => {
      const res = await apiRequest(
        "GET",
        API_CONFIG.lookup.byCategory(selectedCategoryId),
      );
      if (!res.ok) throw new Error(`${res.status}`);
      const payload = await res.json();
  ...
  const {
    ...
    queryFn: async () => {
      const res = await apiRequest(
        "GET",
        API_CONFIG.systemContent.byLookupId(selectedLookupId),
      );
      if (!res.ok) throw new Error(`${res.status}`);
      const payload = await res.json();
            ...
            <div className="flex flex-col sm:flex-row gap-4">
              ...
              <div className="flex-1 space-y-2">
                ...
                <Select
                  disabled
                  value={
                    selectedCategoryId > 0 ? String(selectedCategoryId) : ""
                  }
                  onValueChange={(v) => {
